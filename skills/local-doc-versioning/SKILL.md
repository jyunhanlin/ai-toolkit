...
1. Use **Read** to load the template config from the skill's `template/config.json`
...
For `plan` and `apply` commands that derive code changes from doc diffs, see [experimental/plan-apply.md](experimental/plan-apply.md).
